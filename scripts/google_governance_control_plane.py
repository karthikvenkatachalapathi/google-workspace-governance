...
async function mapProfilesToToken(){const btn=$('mapProfilesBtn'); try{btn.disabled=true;btn.textContent='Mapping…'; const token_ids=selectedMapTokens(); const profiles=selectedMapProfiles(); if(!token_ids.length) throw new Error('Select at least one workspace token.'); if(!profiles.length) throw new Error('Select at least one agent profile.'); const j=await api('/api/workspace/access/map-profiles',{token_ids,profiles}); btn.textContent='Mapped ✓'; $('workspaceMsg').className='msg ok'; $('workspaceMsg').textContent=`Mapped ${token_ids.length} token(s) to ${(j.profiles||profiles).join(', ')} with routes ${Object.values(j.routes||{}).join(', ')}. Review the ACL rows before broad use.`; await load(); await loadWorkspaceAccess(); setTimeout(()=>{btn.disabled=false;btn.textContent='Create / update selected routes'},1200)}catch(e){btn.disabled=false;btn.textContent='Retry mapping';$('workspaceMsg').className='msg error';$('workspaceMsg').textContent=e.message}}
