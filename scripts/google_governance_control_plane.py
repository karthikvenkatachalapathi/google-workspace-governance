...
def _approval_bulk_decide_ui(payload: dict[str, Any], actor: str) -> dict[str, Any]:
    ...
    for approval_id in ids:
        ...
        if decision == "approve_once":
            ...
            continue
        _append_approval_event({"event": "decided", "approval_id": approval_id, "decision": decision, "approver": actor, "decision_reason": str(payload.get("reason") or "bulk UI action"), "approved_until": None})
        changed.append(approval_id)
    _append_change_event({"event": "approval_bulk_decided_from_ui", "actor": actor, "decision": decision, "count": len(changed), "failures": len(failures), "source": "gateway" if decision == "approve_once" else "local_approval_store"})
    return {"status": "ok" if not failures else "partial", "decision": decision, "count": len(changed), "approval_ids": changed, "executed": executed, "failures": failures, "source": "gateway" if decision == "approve_once" else "local_approval_store"}
...
def _approval_channels_list() -> dict[str, Any]:
    ...
    return {"status": "ok", "channels": channels, "settings": settings}
...
def _approval_channel_save(payload: dict[str, Any], actor: str) -> dict[str, Any]:
    ...
    agent_ids = ["*"] if scope == "all" else [x.strip() for x in re.split(r"[,\n]", profile) if x.strip() and x.strip() != "*"]
    if not agent_ids:
        raise ValueError("at least one agent entity/profile is required for this governance principal")
    with _approval_db() as conn:
        ...
        conn.execute("INSERT INTO approval_tenants(id,label,enabled) VALUES(?,?,?) ON CONFLICT(id) DO UPDATE SET label=excluded.label,enabled=excluded.enabled,updated_at=CURRENT_TIMESTAMP", (tenant_id, tenant_label, enabled))
        ...
        conn.commit()
    _append_change_event({"event": "approval_tenant_saved", "actor": actor, "tenant_id": tenant_id, "tenant_label": tenant_label, "chat_id_hash": hashlib.sha256(chat_id.encode()).hexdigest()[:16], "agent_ids": agent_ids, "enabled": bool(enabled), "tenant_bot_token_configured": bool(bot_token), "tenant_bot_token_cleared": clear_bot_token})
    return _approval_channels_list()
...
/* 2026-07 grayscale-icons-gateway-step-tabs: keep all iconography neutral and mirror Workspace step bubbles for MCP Configuration. */
...
let data={rules:[],resources:[],access_log:[],summary:{decisions:{}},control:{}}, users=[], mcpData={tools:[],routes:[]}, active='rules', settingsActive='profile', settingsMode='user', workspaceActive='auth', runtimeActive='status', credentialActive='gateway', settingsNavCollapsed=localStorage.ggovSettingsNavCollapsed==='1', adminSettingsExpanded=localStorage.ggovAdminSettingsExpanded!=='0', gatewaySetupExpanded=localStorage.ggovGatewaySetupExpanded!=='0', apiTokenFilters={}, apiTokenStatusDefaulted=false, agentTokenStatusDefaulted=false, me=null, selected=new Set(), aclSelectionTouched=false, pending2fa=null, pendingTotpChallenge='', sortState={rules:{key:'profile',dir:1},access:{key:'ts',dir:-1},users:{key:'first_name',dir:1},workspaceTokens:{key:'email',dir:1},workspaceRoutes:{key:'profile',dir:1},apiTokens:{key:'created_at',dir:-1},agentTokens:{key:'created_at',dir:-1}}, clientSecretJson=''; const $=id=>document.getElementById(id); const esc=s=>String(s??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch])); const norm=s=>String(s??'').trim(); const uniq=a=>{const seen=new Set(),out=[]; for(const x of a){const v=norm(x); const k=v.toLowerCase(); if(seen.has(k)) continue; seen.add(k); out.push(v)} return out.sort((a,b)=>a.localeCompare(b));}; const rowKey=r=>[r.profile,r.scope,r.resource_alias,r.action].join('|');
...
function isAdmin(){return me&&me.role==='admin'} function updateAdminVisibility(){const admin=isAdmin(); if($('adminSettings')) $('adminSettings').classList.toggle('hidden',!admin); if($('tab-adminSettings')){const adminToggle=$('tab-adminSettings');adminToggle.classList.toggle('hidden',!admin);adminToggle.classList.toggle('expanded',!!(admin&&adminSettingsExpanded));adminToggle.setAttribute('aria-expanded',String(!!(admin&&adminSettingsExpanded)));} if($('tab-gatewaySetup')){const setupToggle=$('tab-gatewaySetup');setupToggle.classList.toggle('hidden',!admin);setupToggle.classList.toggle('expanded',!!(admin&&gatewaySetupExpanded));setupToggle.setAttribute('aria-expanded',String(!!(admin&&gatewaySetupExpanded)));} document.querySelectorAll('.adminSubItem').forEach(b=>b.classList.toggle('hidden',!(admin&&active==='settings'&&settingsMode==='admin'&&adminSettingsExpanded))); document.querySelectorAll('.setupSubItem').forEach(b=>b.classList.toggle('hidden',!(admin&&gatewaySetupExpanded))); if($('tab-approvals')) $('tab-approvals').classList.toggle('hidden',!admin); if(!admin){settingsMode='user'; settingsActive='profile';} const adminMode=admin&&settingsMode==='admin'; const userMode=settingsMode!=='admin'; if($('settingsNav-profile')) $('settingsNav-profile').classList.toggle('hidden',!userMode); ['users','workspace','agents','runtime','tokens'].forEach(s=>{const b=$('settingsNav-'+s); if(b)b.classList.toggle('hidden',!adminMode)}); ['overview','auth','profiles'].forEach(s=>{const b=$('workspaceTab-'+s); if(b)b.classList.toggle('hidden',!adminMode)}); ['status','validation','backups','paths','upgrade'].forEach(s=>{const b=$('runtimeTab-'+s); if(b)b.classList.toggle('hidden',!adminMode)}); if(userMode) settingsActive='profile'; if(adminMode&&settingsActive==='profile') settingsActive='tokens';} function updateUserChrome(){if(!me)return; const name=me.display_name||me.username; if($('userMenuName'))$('userMenuName').textContent=name; updateWelcomeTitle(); if($('userDropdownName'))$('userDropdownName').textContent=name; if($('userDropdownRole'))$('userDropdownRole').textContent=me.role; if($('userMenuAvatar')){$('userMenuAvatar').classList.toggle('hidden',!me.avatar_url); if(me.avatar_url)$('userMenuAvatar').src=me.avatar_url;} if($('settingsMe'))$('settingsMe').textContent=name; if($('settingsRole'))$('settingsRole').textContent=me.role; if($('profileFirst'))$('profileFirst').value=me.first_name||''; if($('profileLast'))$('profileLast').value=me.last_name||''; if($('profileEmail'))$('profileEmail').value=me.email||''; if($('profilePhotoPreview')){$('profilePhotoPreview').classList.toggle('hidden',!me.avatar_url); if(me.avatar_url)$('profilePhotoPreview').src=me.avatar_url;} updateTwofaUi(); updateAdminVisibility();}
...
async function rotateJwtSecret(){if(!confirm('Legacy JWT signing is disabled. Continue to verify disabled status?'))return; try{const j=await api('/api/runtime/jwt-secret/rotate',{}); $('runtimeMsg').className='msg ok';$('runtimeMsg').textContent=`Legacy JWT signing is ${j.storage}. No secret was created or revealed.`; await loadRuntimeStatus();}catch(e){$('runtimeMsg').className='msg error';$('runtimeMsg').textContent=e.message}}
...
function renderApprovalChannels(rows){const tb=$('approvalChannels'); if(!tb)return; const rowsArr=rows||[]; if($('telegramDestSummary')){const active=rowsArr.filter(c=>c.enabled).length; const tenantBots=rowsArr.filter(c=>c.bot_token_configured).length; $('telegramDestSummary').textContent=`${active} active / ${rowsArr.length} approval user${rowsArr.length===1?'':'s'}${tenantBots?` · ${tenantBots} dedicated bot${tenantBots===1?'':'s'}`:''}`;} tb.innerHTML=rowsArr.map(c=>{const applies=c.scope==='all'?'All agent entities':`Agent: ${esc(c.profile||'—')}`; const id=Number(c.id); const bot=c.bot_token_configured?'<span class="pill ok">governance bot</span>':'<span class="pill warn">bot missing</span>'; const approvalUser=esc(c.tenant_label||c.label||'Governance user'); return `<tr data-channel-row="${id}"><td><b>${approvalUser}</b><div class="muted">${esc(c.label||'Approver')}</div></td><td class="code">${esc(c.chat_id)}</td><td>${applies}</td><td>${bot}</td><td><label class="miniSwitch"><input type="checkbox" ${c.enabled?'checked':''} onchange="toggleApprovalChannel(${id},this.checked)"/><span></span><b>${c.enabled?'enabled':'disabled'}</b></label></td><td><div class="channelActionButtons"><button class="iconBtn channelIconBtn dangerBtn" type="button" title="Delete governance bot configuration" aria-label="Delete governance bot configuration for ${approvalUser}" onclick="deleteApprovalChannel(${id})">🗑</button></div></td></tr>`}).join('')||'<tr><td colspan="6" class="muted">No governance approval users configured.</td></tr>';}
async function saveApprovalChannel(){try{const payload={tenant_label:$('channelLabel').value,label:$('channelLabel').value,chat_id:$('channelChatId').value,scope:$('channelScope').value,profile:$('channelProfile').value,bot_token:$('channelBotToken')?$('channelBotToken').value:'',clear_bot_token:false,enabled:$('channelEnabled').value==='true'}; const j=await api('/api/approval-channels/save',payload); ['channelLabel','channelChatId','channelBotToken'].forEach(id=>{if($(id))$(id).value=''}); $('channelMsg').className='msg ok';$('channelMsg').textContent='Governance approval user saved.'; renderApprovalSettings(j.settings||{},j.channels||[]); renderApprovalChannels(j.channels||[]);}catch(e){$('channelMsg').className='msg error';$('channelMsg').textContent=e.message}}
async function toggleApprovalChannel(id,enabled){try{const rows=(await api('/api/approval-channels/list',{})).channels||[]; const c=rows.find(x=>Number(x.id)===Number(id)); if(!c)throw new Error('Approval user not found'); const j=await api('/api/approval-channels/save',{id,tenant_id:c.tenant_id,tenant_label:c.tenant_label||c.label,label:c.label,chat_id:c.chat_id,scope:c.scope,profile:c.profile,bot_token:'',clear_bot_token:false,enabled}); renderApprovalSettings(j.settings||{},j.channels||[]); renderApprovalChannels(j.channels||[]); if($('channelMsg')){$('channelMsg').className='msg ok';$('channelMsg').textContent=`Approval route ${enabled?'enabled':'disabled'}.`;}}catch(e){if($('channelMsg')){$('channelMsg').className='msg error';$('channelMsg').textContent=e.message}; await loadApprovalChannels();}}
async function deleteApprovalChannel(id){try{const j=await api('/api/approval-channels/delete',{id}); $('channelMsg').className='msg ok';$('channelMsg').textContent='Approval user deleted.'; renderApprovalSettings(j.settings||{},j.channels||[]); renderApprovalChannels(j.channels||[]);}catch(e){$('channelMsg').className='msg error';$('channelMsg').textContent=e.message}}
