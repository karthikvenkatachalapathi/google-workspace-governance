...
CONTROL_ASSET_BASE = Path(os.getenv("GOOGLE_GOVERNANCE_CONTROL_ASSET_DIR", str(SELF_CONTAINED_BASE / "generated/ui/control-plane")))
CONTROL_LOGO_PATH = Path(os.getenv("GOOGLE_GOVERNANCE_CONTROL_LOGO", str(CONTROL_ASSET_BASE / "google-agent-gateway-logo.jpg")))
CONTROL_LOGO_LIGHT_PATH = Path(os.getenv("GOOGLE_GOVERNANCE_CONTROL_LOGO_LIGHT", str(CONTROL_ASSET_BASE / "google-agent-gateway-logo-light.png")))
CONTROL_LOGO_DARK_PATH = Path(os.getenv("GOOGLE_GOVERNANCE_CONTROL_LOGO_DARK", str(CONTROL_ASSET_BASE / "google-agent-gateway-logo-dark.png")))
CONTROL_LOGIN_LOGO_DARK_PATH = Path(os.getenv("GOOGLE_GOVERNANCE_CONTROL_LOGIN_LOGO_DARK", str(CONTROL_ASSET_BASE / "google-agent-gateway-logo-login-dark.png")))
CONTROL_ICON_LIGHT_PATH = Path(os.getenv("GOOGLE_GOVERNANCE_CONTROL_ICON_LIGHT", str(CONTROL_ASSET_BASE / "google-agent-gateway-icon-light.png")))
CONTROL_ICON_DARK_PATH = Path(os.getenv("GOOGLE_GOVERNANCE_CONTROL_ICON_DARK", str(CONTROL_ASSET_BASE / "google-agent-gateway-icon-dark.png")))
CONTROL_FAVICON_PATH = Path(os.getenv("GOOGLE_GOVERNANCE_CONTROL_FAVICON", str(CONTROL_ASSET_BASE / "favicon.ico")))
...
def _runtime_backup_item_from_archive(archive: Path, seen: set[str]) -> dict[str, Any] | None:
    try:
        if not archive.exists() or not archive.is_file() or str(archive) in seen:
            return None
        ...
        for suffix in (".tar.gz", ".tgz"):
            if backup_id.endswith(suffix):
                ...
                break
        backup_dir = archive.with_suffix("")
        if archive.name.endswith(".tar.gz"):
            backup_dir = Path(str(archive)[: -len(".tar.gz")])
        ts = datetime.fromtimestamp(archive.stat().st_mtime, timezone.utc).isoformat()
        manifest = backup_dir / "manifest.json"
        if manifest.exists():
            try:
                meta = json.loads(manifest.read_text(encoding="utf-8") or "{}")
                backup_id = str(meta.get("id") or backup_id)
        ...
        return {
            ...
            "includes_token_store": False,
            "status": "discovered",
            "note": "Discovered on disk; not yet recorded in the control database.",
            "archive_exists": True,
...
def _runtime_backups() -> list[dict[str, Any]]:
    ...
    try:
        with _control_db() as conn:
            for row in conn.execute("SELECT id,ts,actor,archive_path,backup_dir,includes_token_store,status,note FROM runtime_backups ORDER BY ts DESC LIMIT 50").fetchall():
                item = dict(row)
        ...
        pass
    try:
        if RUNTIME_BACKUP_ROOT.exists():
            for archive in sorted([*RUNTIME_BACKUP_ROOT.rglob("*.tar.gz"), *RUNTIME_BACKUP_ROOT.rglob("*.tgz")], key=lambda p: p.stat().st_mtime, reverse=True):
                item = _runtime_backup_item_from_archive(archive, seen)
                if item:
                    rows.append(item)
                    seen.add(str(archive))
    except Exception:
        pass
    return sorted(rows, key=lambda x: str(x.get("ts") or ""), reverse=True)[:50]
...
def _runtime_backup_schedule_status() -> dict[str, Any]:
    ...
    content = RUNTIME_BACKUP_CRON_PATH.read_text(encoding="utf-8") if exists else ""
    cron = ""
    include_token_store = False
    if content:
        for line in content.splitlines():
            ...
            if len(parts) >= 6:
                cron = " ".join(parts[:5])
                include_token_store = "--include-token-store" in parts[5]
                break
    return {
        "enabled": False,
        "configured": exists,
        "installed": False,
        "cron": cron,
        "cron_path": str(RUNTIME_BACKUP_CRON_PATH),
        "include_token_store": include_token_store,
        "content": content,
        "message": "Schedule file is prepared for operator installation; the Control UI does not install a system crontab/timer.",
    }
...
def _runtime_backup_schedule(payload: dict[str, Any] | None = None, actor: str = "admin") -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": status, "enabled": False, "configured": enabled, "installed": False, "cron": expr if enabled else "", "cron_path": str(RUNTIME_BACKUP_CRON_PATH), "include_token_store": include_token_store if enabled else False, "content": content if enabled else "", "message": "Schedule file prepared; install it with crontab or a systemd timer to activate recurring backups."}
    except PermissionError:
...
<html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Google Governance Control</title><link id="faviconLight" rel="icon" type="image/png" sizes="184x184" href="/assets/icon-light.png?v=29"/><link id="faviconDark" rel="icon" type="image/png" sizes="184x184" href="/assets/icon-dark.png?v=29"/><link rel="shortcut icon" href="/favicon.ico?v=29"/><link rel="apple-touch-icon" href="/assets/icon-light.png?v=29"/><style>
...
/* 2026-07 runtime-backups-postgres-tab-fix: Postgres has its own tab; backup inventory table spans the full content view; cron schedules render parsed cron and source file. */
#runtimePane-backups .backupGrid{grid-template-columns:repeat(auto-fit,minmax(min(360px,100%),1fr))!important}
#runtimePane-backups .backupStatusGrid{display:grid!important;grid-template-columns:1fr!important;gap:16px!important;width:100%!important}
#runtimePane-backups #runtimeBackups.backupInventoryFull{grid-column:1/-1!important;width:100%!important;max-width:none!important;overflow:auto!important}
.backupInventoryTable{width:100%!important;min-width:1120px!important;table-layout:fixed!important;border-collapse:separate!important;border-spacing:0!important}
.backupInventoryTable th,.backupInventoryTable td{vertical-align:top!important;overflow-wrap:anywhere!important;word-break:break-word!important}
.backupInventoryTable th:nth-child(1),.backupInventoryTable td:nth-child(1){width:19%!important}
.backupInventoryTable th:nth-child(2),.backupInventoryTable td:nth-child(2){width:31%!important}
.backupInventoryTable th:nth-child(3),.backupInventoryTable td:nth-child(3){width:28%!important}
.backupInventoryTable th:nth-child(4),.backupInventoryTable td:nth-child(4){width:8%!important;white-space:nowrap!important}
.backupInventoryTable th:nth-child(5),.backupInventoryTable td:nth-child(5){width:8%!important}
.backupInventoryTable th:nth-child(6),.backupInventoryTable td:nth-child(6){width:6%!important}
.postgresPane .postgresMigrationGrid{display:grid!important;grid-template-columns:minmax(360px,520px) minmax(0,1fr)!important;gap:16px!important;align-items:start!important}
.postgresPane .postgresMigrationCard,.postgresPane .postgresStatusFull{width:100%!important;box-sizing:border-box!important}
@media(max-width:980px){.postgresPane .postgresMigrationGrid{grid-template-columns:1fr!important}.backupInventoryTable{min-width:860px!important}}

/* 2026-07 user-supplied-governance-brand-v27: theme-aware attached logos and icons. */
.brandCluster .textBrandLink{display:flex!important;align-items:center!important;width:100%!important;height:var(--topBarH)!important;padding:0 10px!important;overflow:hidden!important;text-decoration:none!important;background:transparent!important}
.brandCluster .googleTextBrand{display:none!important}
.brandCluster img.themeLogo.brandLogo{display:block!important;width:min(232px,calc(var(--leftPaneW) - 20px))!important;max-width:calc(var(--leftPaneW) - 20px)!important;height:auto!important;max-height:76px!important;object-fit:contain!important;object-position:left center!important;aspect-ratio:auto!important;margin:0!important;background:transparent!important;transform:none!important;filter:none!important;mix-blend-mode:normal!important}
body.light .brandCluster img.themeLogo.lightLogo.brandLogo{display:block!important}
body.light .brandCluster img.themeLogo.darkLogo.brandLogo{display:none!important}
body:not(.light) .brandCluster img.themeLogo.darkLogo.brandLogo{display:block!important}
body:not(.light) .brandCluster img.themeLogo.lightLogo.brandLogo{display:none!important}
body.mainNavCollapsed .brandCluster img.themeLogo.brandLogo{display:none!important}
body.authing .brandCluster img.themeLogo.brandLogo{display:none!important}
body.authing #loginView .loginBrand > img.themeLogo.loginHeroLogo{width:min(470px,78vw)!important;max-width:100%!important;height:auto!important;max-height:none!important;object-fit:contain!important;object-position:left center!important;aspect-ratio:auto!important;margin:0!important;transform:none!important;background:transparent!important;filter:none!important;mix-blend-mode:normal!important}
body.light.authing #loginView .loginBrand > img.themeLogo.lightLogo.loginHeroLogo{display:block!important}
body.light.authing #loginView .loginBrand > img.themeLogo.darkLogo.loginHeroLogo{display:none!important}
body:not(.light).authing #loginView .loginBrand > img.themeLogo.darkLogo.loginHeroLogo{display:block!important}
body:not(.light).authing #loginView .loginBrand > img.themeLogo.lightLogo.loginHeroLogo{display:none!important}
...
function showRuntimePane(){['status','validation','backups','postgres','paths','upgrade'].forEach(s=>{const pane=$('runtimePane-'+s); if(pane)pane.classList.toggle('hidden',runtimeActive!==s); const b=$('runtimeTab-'+s); if(b)b.classList.toggle('active',runtimeActive===s); const tb=$('runtimeTop-'+s); if(tb)tb.classList.toggle('active',runtimeActive===s);});}
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path == "/assets/logo-login-dark.png" and CONTROL_LOGIN_LOGO_DARK_PATH.exists():
                ...
                return
            if path == "/assets/icon-light.png" and CONTROL_ICON_LIGHT_PATH.exists():
                _bytes_response(self, 200, CONTROL_ICON_LIGHT_PATH.read_bytes(), "image/png")
                return
            if path == "/assets/icon-dark.png" and CONTROL_ICON_DARK_PATH.exists():
                _bytes_response(self, 200, CONTROL_ICON_DARK_PATH.read_bytes(), "image/png")
                return
            if path == "/favicon.ico":
                if CONTROL_FAVICON_PATH.exists():
                    _bytes_response(self, 200, CONTROL_FAVICON_PATH.read_bytes(), "image/x-icon")
                    return
                if CONTROL_ICON_LIGHT_PATH.exists():
                    _bytes_response(self, 200, CONTROL_ICON_LIGHT_PATH.read_bytes(), "image/png")
                    return
                if CONTROL_ICON_DARK_PATH.exists():
                    _bytes_response(self, 200, CONTROL_ICON_DARK_PATH.read_bytes(), "image/png")
                    return
