...
Explore the inert static demo at [governance-gateway-demo.pages.dev](https://governance-gateway-demo.pages.dev/). The demo is a product walkthrough with mock data only; it does not connect to Google, send requests to a live gateway, or expose real credentials.
...
The native installer is self-contained: runtime copy, venv, SQLite state, OAuth custody, setup secrets, logs, and backups live under `./.google-governance/` plus `./database/` inside the clone. Those paths are runtime state and are ignored by Git; they are not part of the published repository. The only host-level install artifacts are the systemd units plus the dedicated service user/group.
...
The admin UI/API is the source of truth for workspace connections, routes, ACLs, approvals, and runtime policy generation. Runtime SQLite databases, OAuth custody, generated policy state, backups, and logs stay in ignored local state directories, not in Git. Optional Postgres migration/cutover steps are documented in [SETUP.md](SETUP.md#13-optional-postgres-migration-and-operator-cutover); the default install includes Postgres driver/backend wiring while keeping SQLite active until explicitly configured.
...
| `.google-governance/state/policy/profile_policy.json` | Ignored local runtime ACL policy generated by the admin UI |
| `.google-governance/state/policy/generated_profile_policy.json` | Ignored local generated policy snapshot for validation/recovery |
